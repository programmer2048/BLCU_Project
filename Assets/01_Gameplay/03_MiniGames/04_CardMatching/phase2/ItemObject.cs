...
public class ItemObject : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    ...
    public void UpdateVisual(bool useAnimation)
    {
        ...
        else
        {
            switch (type)
            {
                ...
                case ItemType.Fragment:
                    frameImage.sprite = fragmentFrame; // 确保在Inspector里拖入了碎片背景图
                    break;
        ...
        // 2. 刷新缩放（如果是碎片，可以稍微缩小一点 icon，显得在框内）
        if (useAnimation)
        {
            transform.DOPunchScale(Vector3.one * 0.1f, 0.2f);
        }

        // 3. 处理碎片 icon 的特殊显示（可选：如果碎片 icon 需要变暗或变色）
        if (iconImage != null)
        {
