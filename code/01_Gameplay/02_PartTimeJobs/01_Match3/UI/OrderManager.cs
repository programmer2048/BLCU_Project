...
public class OrderManager : MonoBehaviour
{
    ...
    [Header("References")]
    public GameObject trayPrefab;      // 托盘Prefab
    ...
    void Update()
    {
        ...
        {
            _timer = 0;
            spawnInterval = UnityEngine.Random.Range(1.5f,5f);
            GenerateNewOrder();
        }
    }
    ...
    public void OnOrderFailed(TrayController tray)
    {
        ...
        M3_GameManager.Instance.ModifyHealth(-1*(tray.requiredCount-tray.currentCount)); // 扣血
        //Debug.Log(M3_GameManager.Instance.currentHealth);
        if (activeOrders.Contains(tray))
        {
            activeOrders.Remove(tray);
