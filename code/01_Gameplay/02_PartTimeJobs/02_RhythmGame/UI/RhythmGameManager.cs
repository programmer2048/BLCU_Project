...
public class RhythmGameManager : MonoBehaviour
{
    ...
    public TextMeshProUGUI finalScoreReward; // 获得的旅费
    ...
    IEnumerator LoadResourcesFromDisk() { yield break; } 
    IEnumerator LoadAudioRoutine(string f) { yield break; }
    ...
    IEnumerator GameOverRoutine()
    {
        ...
        if (finalScoreReward) finalScoreReward.text = $"获得旅费：{Mathf.FloorToInt(currentScore * 0.2f)}";
    ...
    void AddRevenueToSave()
    {
        ...
        if (SaveManager.Instance != null)
        {
            ...
            bankHistory.chatLog.Add(new ChatMessage { sender = SenderType.System, type = MessageType.SystemAlert, content = $"【远山民宿】到账 ${amount}。", timeStamp = "Now" });
    ...
    void SpawnNoteObject(ChartNote data)
    {
        ...
        if (data.type == "trap") return; // 暂停渲染"陷阱"
    ...
    {
        //if (lane < laneTouchFeedbacks.Length && laneTouchFeedbacks[lane]) laneTouchFeedbacks[lane].CrossFadeAlpha(0f, 0.3f, false);
    }
    void ProcessLaneAlpha(int lane, KeyControl key)
    {
        if (lane >= laneTouchFeedbacks.Length || laneTouchFeedbacks[lane] == null) return;

        // 获取当前按键是否处于“按住”状态 (isPressed)
        ...
        {
            // 如果没按住，获取当前透明度并平滑插值到 0
            float currentAlpha = laneTouchFeedbacks[lane].canvasRenderer.GetAlpha();
            if (currentAlpha > 0f)
            {
                float newAlpha = Mathf.MoveTowards(currentAlpha, 0f, Time.deltaTime * 10f);
                laneTouchFeedbacks[lane].canvasRenderer.SetAlpha(newAlpha);
            }
        }
    }
